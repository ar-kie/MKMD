...
#Updated some figures 12/17/2021


#==========================================================================================================#
#                                   RNA-seq analysis start: normalization                                  #
#==========================================================================================================#
#Subsetting a df to samples of interest (D20)
#rownames(gfilt.df) <- make.unique(gsub("__.*","",rownames(gfilt.df)))

subset.df <- gfilt.df[colnames(gfilt.df) %in% rownames(filt.meta[filt.meta$DIV %in% c("W4"),])]
...
cov_for.cor.sub <- cov_for.cor[cov_for.cor$DIV %in% c("2"),] #chooses week 2 samples
...
#D20 PCA-covariate analysis
#500x500
D20.PCAcovar <- PCA.covar(subset.df,cov_for.cor.sub,data.r=T,type="pearson")
...
D20.PCAcovar.postcorrection <- PCA.covar(batch.rem,covs_forremBatch,data.r=T,type="pearson",heatmap.r=T)
...
downreg.genes <- rownames(res.complex[res.complex$log2FoldChange < -0.5 & res.complex$padj < 0.05,])
upreg.genes <- rownames(res.complex[res.complex$log2FoldChange > 0.5 & res.complex$padj < 0.05,])


#Only most significant genes
write.xlsx2(res.complex[res.complex$log2FoldChange < 0 & res.complex$padj < 0.05,], 
            file = "04012021_D20_DEGs_down.xlsx", sheetName = "Down",
            col.names = TRUE, row.names = TRUE, append = FALSE)
write.xlsx2(res.complex[res.complex$log2FoldChange > 0 & res.complex$padj < 0.05,], 
            file = "04012021_D20_DEGs_up.xlsx", sheetName = "Up",
            col.names = TRUE, row.names = TRUE, append = FALSE)
...
pheatmap(sub, 
         ...
         annotation_col=annot_df[-2], fontsize_row = 5.5,
...
res.sorted.up <- res.sorted.up[res.sorted.up$log2FoldChange > 0.5 & res.sorted.up$padj < 0.05,]


res.sorted.down <- res.complex[order(res.complex$log2FoldChange, decreasing = F),]
res.sorted.down <- res.sorted.down[res.sorted.down$log2FoldChange < -0.5 & res.sorted.down$padj < 0.05,]
...
#550x600
ComplexHeatmap::pheatmap(top.sub, 
         cluster_rows = T, 
         cluster_cols = F, annotation_legend = T, show_colnames = F,
...
#                x = 'log2FoldChange',
#                y = 'padj',FCcutoff = 0.5, pCutoff = 0.05)
pdf(file="04022021_D20_volcano_new thresh.pdf")
...
#==========================================================================================================#
#Boxplot for mTOR
box <- data.frame(scale(t(sorted)), `Cell line`=annot_df[order(fact.to.sort, decreasing=F),][2], check.names=F)
MTOR <- box[colnames(box) %in% c("MTOR","Cell line")]
ggplot(MTOR, aes_string(x="`Cell line`", y="MTOR", fill="`Cell line`")) +
  geom_boxplot() +
  scale_fill_manual(values = c("#00AFBB", "#E7B800", "#FC4E07"))+
  #  theme_ipsum() +
...
box <- data.frame(scale(t(gfilt.df)), `DIV`=filt.meta$DIV, `Cell line`=filt.meta$`Cell line`, check.names=F)

MTOR <- box[colnames(box) %in% c("MTOR","DIV","Cell line")]
ggplot(MTOR, aes_string(x="DIV", y="MTOR", fill="DIV")) +
  geom_boxplot() +
  scale_fill_manual(values = c("#00AFBB", "#E7B800", "#FC4E07"))+
  #  theme_ipsum() +
...
ggplot(MTOR, aes_string(x="DIV", y="MTOR")) +
  geom_boxplot() +
  #  scale_fill_manual(values = c("#00AFBB", "#E7B800", "#FC4E07"))+
  #  theme_ipsum() +
  geom_point(aes(shape=`Cell line`))+
...
write.xlsx2(res.complex, 
            file = "04012021_D20_all_DEGs.xlsx", sheetName = "D20 all",
            col.names = TRUE, row.names = TRUE, append = FALSE, check.names=F)

write.xlsx2(data.frame(batch.rem,check.names=F), 
            file = "04012021_D20_vst_corrected.xlsx", sheetName = "vst-normalized & corrected",
            col.names = TRUE, row.names = TRUE, append = FALSE)
write.table(data.frame(batch.rem,check.names=F), file = "04022021_D20_vst normalized_corrected.txt")
#options(java.parameters = "-Xmx20000m")
#Extracting the normalized values
write.table(gfilt.df,file = "04022021_filtered_raw.txt")
write.table(t(scale(t(gfilt.df))),file = "04022021_filtered_scaled.txt")


#For mTOR & P70S6K (RPS6KB1)
#Raw
write.xlsx2(data.frame(batch.rem,check.names=F)[rownames(batch.rem) %in% c("MTOR", "RPS6KB1"),], 
            file = "04012021_D20_mTOR_vst_corrected.xlsx", sheetName = "mTOR & P70S6K vst-norm and corrected",
            col.names = TRUE, row.names = TRUE, append = FALSE)

#Filtered
write.xlsx2(data.frame(gfilt.df,check.names=F)[rownames(gfilt.df) %in% c("MTOR", "RPS6KB1"),], 
            file = "04012021_D20_mTOR_filtered_raw.xlsx", sheetName = "mTOR & P70S6K raw",
            col.names = TRUE, row.names = TRUE, append = FALSE)
