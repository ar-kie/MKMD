...
#==========================================================================================================#
#Subsetting a df to samples of interest (D35)
rownames(gfilt.df) <- make.unique(gsub("__.*","",rownames(gfilt.df)))

subset.df <- gfilt.df[colnames(gfilt.df) %in% rownames(filt.meta[filt.meta$DIV %in% c("W5"),])]
...
cov_for.cor.sub <- cov_for.cor[cov_for.cor$DIV %in% c("3"),]
...
#D35 PCA-covariate analysis
#500x500
D35.PCAcovar <- PCA.covar(subset.df,cov_for.cor.sub,data.r=T,type="pearson")

#For correcting the data
subset.meta$'Log(library size)' <- scale(subset.meta$`Library size`) #was log-transformed earlier
...
D35.PCAcovar.postcorrection <- PCA.covar(batch.rem,covs_forremBatch,data.r=T,type="pearson",heatmap.r=T)
...
write.xlsx2(res.complex[res.complex$log2FoldChange < 0 & res.complex$padj < 0.05,], 
            file = "04012021_D35_DEGs_down.xlsx", sheetName = "Down",
            col.names = TRUE, row.names = TRUE, append = FALSE)
write.xlsx2(res.complex[res.complex$log2FoldChange > 0 & res.complex$padj < 0.05,], 
            file = "04012021_D35_DEGs_up.xlsx", sheetName = "Up",
            col.names = TRUE, row.names = TRUE, append = FALSE)
...
top.sub <- sorted[rownames(sorted) %in% c(top.upreg.genes,downreg.genes),]
#550x600
pheatmap(top.sub, 
         cluster_rows = T, 
         cluster_cols = F, annotation_legend = T, show_colnames = F,
...
#                x = 'log2FoldChange',
#                y = 'pvalue',FCcutoff = 2)
pdf(file="04022021_D35_volcano_new thresh.pdf")
...
#Boxplots of DEGs
library(DEGreport)
degPlot(dds = dds.complex, res = res.complex, n = 6, xs = "stimulation")

degPlotWide(dds.complex, "MTOR", group="Stimulation")
...
write.xlsx2(res.complex, 
            file = "04012021_D35_all_DEGs.xlsx", sheetName = "D35 all",
...
write.xlsx2(data.frame(batch.rem,check.names=F), 
            file = "04012021_D35_vst_corrected.xlsx", sheetName = "vst-normalized & corrected",
            col.names = TRUE, row.names = TRUE, append = FALSE)

#For mTOR & P70S6K (RPS6KB1)
#Raw
write.xlsx2(data.frame(batch.rem,check.names=F)[rownames(batch.rem) %in% c("MTOR", "RPS6KB1"),], 
            file = "04012021_D35_mTOR_vst_corrected.xlsx", sheetName = "mTOR & P70S6K vst-norm and corrected",
            col.names = TRUE, row.names = TRUE, append = FALSE)

#Filtered
write.xlsx2(data.frame(gfilt.df,check.names=F)[rownames(gfilt.df) %in% c("MTOR", "RPS6KB1"),], 
            file = "04012021_D35_mTOR_filtered_raw.xlsx", sheetName = "mTOR & P70S6K raw",
            col.names = TRUE, row.names = TRUE, append = FALSE)
